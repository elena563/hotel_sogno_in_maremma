...
function showWindow() {
  ...
  windows.forEach((window) => {
    ...
    setTimeout(() => {
      // Dopo un breve ritardo, applica la transizione
...
function getprice() {
  ...
  for (let el in roomRates) {
    if (people > 2 && el === "jacuzzi") {
      // jacuzzi room only for two people
    ...
    let price =
      ...
      basePrice *
        (10 / 100) *
        (selected.adults - 1) /*increase 10% for every adult*/ +
...
function displayMessage(el) {
  ...
  if (priceCont) {
    priceCont.textContent =
      "Il numero di ospiti per stanza è troppo elevato. Considera di aggiungere stanze o ridurre il numero di ospiti.";
...
function displayUnavailable(el) {
  ...
  priceConts.forEach((priceCont) => {
    if (priceCont.classList.contains(el)) {
      if (el === "jacuzzi") {
        priceCont.textContent =
          "Questa stanza non è disponibile per più di 2 ospiti";
